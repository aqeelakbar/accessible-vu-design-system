...
The repository is set up around Storybook rather than a standalone Vite app; there is no `index.html` entry point for `npm run dev` or `npm run build`. Run the Button test with `npm test -- --run`. Dependencies are installed with `npm ci`; the repository ignores generated `node_modules` files.
